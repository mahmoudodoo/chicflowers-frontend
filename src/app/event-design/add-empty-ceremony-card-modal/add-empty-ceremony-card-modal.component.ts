...
@Component({
  selector: 'app-add-empty-ceremony-card-modal',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './add-empty-ceremony-card-modal.component.html',
  styleUrls: ['./add-empty-ceremony-card-modal.component.css']
})
export class AddEmptyCeremonyCardModalComponent {
  ...
  ngOnInit() {
    // Fetch arrangement types for the ceremony section
  ...
  addCard(): void {
    ...
    this.eventDesignService.addEmptyCeremonyCard(this.eventId, cardTitle, arrangementTypeId).subscribe(
      (response) => {
        if (response.success) {
          console.log('Ceremony card added successfully:', response);
